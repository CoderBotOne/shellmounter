...
use crate::kanban::{Board, Card, Priority, Column};
    ...
    v_flex().size_full().bg(theme.background)
        .child(
            h_flex().px_4().py_2().gap_2().items_center().border_b_1().border_color(theme.border)
                ...
                .child(div().flex_1())
                .child(
                    Button::new("kanban-add").primary().child("+ Card")
                        .on_click(cx.listener(move |this, _, _, cx| {
                            let p = save_path.clone();
                            let mut b = Board::load(&p).unwrap_or_else(|_| Board::new("Termia Board"));
                            let card = Card {
                            ...
                            if let Some(col) = b.columns.first_mut() {
                                col.cards.push(card);
                            }
                            let _ = b.save(&p);
                            this.status_message = "Card added".into();
                            cx.notify();
                        })))
        )
        .child(
