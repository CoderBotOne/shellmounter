    ...
    v_flex().size_full().bg(theme.background)
        .child(
            h_flex().px_4().py_2().gap_2().items_center().border_b_1().border_color(theme.border)
                ...
                .child(div().flex_1())
                .child(
                    Button::new("git-stage-all").ghost().child("Stage all")
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if let Ok(()) = git::stage_all(&rp) {
                                this.status_message = "Staged all".into();
                            } else {
                                this.status_message = "Stage failed".into();
                            }
                            cx.notify();
                        })))
                .child(
                    Button::new("git-commit").primary().child("Commit")
                        .on_click(cx.listener(move |this, _, _, cx| {
                            let rp2 = git::find_repo(&this.data_dir).unwrap_or(this.data_dir.clone());
                            match git::commit(&rp2, "Termia commit") {
                                Ok(sha) => { this.status_message = format!("Committed {}", &sha[..7]); }
                                Err(e) => { this.status_message = format!("Commit failed: {e}"); }
                            }
                            cx.notify();
                        })))
        )
...
fn hsla_color(hex: u32) -> Hsla {
    hsla(((hex >> 16) & 0xff) as f32 / 255.0 * 360.0, 0.7, 0.5, 1.0)
}
